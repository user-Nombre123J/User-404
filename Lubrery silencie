...
UIS.InputBegan:Connect(function(input, gameProcessed)
	if
		input.KeyCode
		== ((typeof(ui_options.toggle_key) == "EnumItem") and ui_options.toggle_key or Enum.KeyCode.RightShift)
	then
	...
	do
		...
		Toggle.MouseButton1Click:Connect(function()
			...
			if is_open then
				...
				Resize(
					Window,
					{ Size = UDim2.new(0, Window.AbsoluteSize.X, 0, options.min_size.Y) },
					options.tween_time
				)
		...
		do
			function window_data:AddTab(tab_name)
				...
				local function updateParticles()
					...
					activeParticleConnection = game:GetService("RunService").Heartbeat:Connect(function()
				...
				do
					...
					function tab_data:AddTextBox(textbox_text, callback, textbox_options)
						...
						callback = typeof(callback) == "function" and callback or function() end
						textbox_options = typeof(textbox_options) == "table" and textbox_options
							or { ["clear"] = false }
						textbox_options = {
										...
										if Entered and not slider_options.readonly then
											while Held and not dropdown_open do
												...
												local x = (
													...
													- (
														slider.AbsoluteSize.X
														- (mouse_location.X - slider.AbsolutePosition.X)
														+ 1
													)
												) / slider.AbsoluteSize.X
								...
								if inputObject.UserInputType == Enum.UserInputType.MouseButton1 then
									...
									spawn(function()
										while Held and Entered1 and not dropdown_open do
											...
											local x = (
												(
													palette.AbsoluteSize.X
													- (mouse_location.X - palette.AbsolutePosition.X)
												) + 1
											)
											local y = (
												(
													palette.AbsoluteSize.Y
													- (mouse_location.Y - palette.AbsolutePosition.Y)
												) + 1.5
											)
											...
											Resize(palette_indicator, {
												Position = UDim2.new(
													0,
													math.abs(x - 100) - (palette_indicator.AbsoluteSize.X / 2),
													0,
					...
					function tab_data:AddConsole(console_options)
						...
						console_options = typeof(console_options) == "table" and console_options
							or { ["readonly"] = true, ["full"] = false }
						console_options = {
						...
						do
							...
							local highlight_lua = function(type)
								...
								local highlight_logs = function(type) end
								if type == "Text" then
									Source.Text = Source.Text:gsub("\13", "")
									Source.Text = Source.Text:gsub("\t", "      ")
									local s = Source.Text
